...
@section('content')
<div>
    <form action="{{Route('addmilk')}}">
        <label for="cat">Введите вид молока</label>
...
<div>
    <table class="table table-striped">
        ...
        <tbody>
            ...
            <tr>
                <th scope="row">{{$milk->id}}</th>
                <td class="profil-p" data-review-content="{{$milk->name}}" id="reviewContent-{{ $milk->id }}">{{$milk->name}}</td>
                <td><button class="btn btn-primary edit-review-btn show-btn"
                    data-toggle="modal"
                    data-target="#editReviewModal"
                    data-review-id="{{$milk->id}}"
                    data-review-content="{{$milk->name}}">
                    Ред.
                    </button></td>
                <td><form action="{{Route('delmilk')}}" method="POST">@csrf
                    @method('DELETE')<button name="id" class="btn btn-primary " type="submit" value="{{$milk->id}}">del</button></form></td>
              </tr>
            @endforeach
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editReviewModalLabel">Редактировать вид молока</h5>
...
@endsection
@section('script')
<script src="/js/redmilk.js"></script>
